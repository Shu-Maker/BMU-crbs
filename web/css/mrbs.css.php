...
td form {margin: 0}     /* Prevent displaying margins around forms in tables. */
...
$admin_form_label_width       = '7.0';   
$admin_form_gap               = '1.0';   
$admin_form_input_width       = '10.5';   
...
<?php
$column_hidden_width  = 0;       // (%) width of the column for hidden days (0 for no column; 1 for a narrow column);
                                 
$column_row_labels_width   = 1;  // (%) width of the row label columns 
$n_hidden_days = count($hidden_days);


$column_month = 100 - ($column_hidden_width *  $n_hidden_days);
if ($n_hidden_days < 7)                                  // (avoid the division by zero)
{
  $column_month = $column_month/(7 - $n_hidden_days);      
}
$column_month = number_format($column_month, 1, '.', '');  
